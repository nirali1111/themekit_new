...
$(document).ready(function(){ 
  ...
  $('.tab-a').click(function(){
    ...
    $(".tab-a[data-id='"+$(this).attr('data-id')+"']").addClass("active");
...
function getCartItems(cartObj)
{
   ...
   console.log(cart);
	var html = '';
    $.each(cart,function(index,citem){
      var addIndex = index+1;
      html += '<tr data-varid="'+citem.id+'" class="trid" line_index="'+addIndex+'">';
      html += '<td><img src="'+citem.image+'" width="80px" height="80px"></td>';
      html += '<td>'+citem.product_title+'<p><a href="javascript:void(0);" onClick="removeRow()" class="cart_remove">REMOVE</a></p></td>';
      html += '<td id="price">'+theme.Currency.formatMoney(citem.price, theme.moneyFormat)+'</td>';
      html += '<td><input type="number" value="'+citem.quantity+'" class="cartqty-change"></td>';
      html += '<td class="total">'+theme.Currency.formatMoney(citem.line_price, theme.moneyFormat)+'</td>';
      html += '</tr>';   
    });
    $('.lineitem').html(html);
}
...
function updatetotal(){
	$(".cartqty-change").change(function() {
          var qty = $(this).val();
          var variant_id = $(this).closest('tr').attr( "data-varid" );
...
function removeRow(){
	$(".cart_remove").click(function() {
          var qty = 0;
          var line_no = $(this).closest('tr').attr( "line_index" );
          console.log(line_no);
           $.ajax({
               type: 'POST',
               url: '/cart/change.js',
               data: {line: line_no, quantity: qty},
               dataType: 'json',
               success: function(data) {
                  jQuery.getJSON('/cart.js', function (cart, textStatus) {
                     getCartItems(cart)
